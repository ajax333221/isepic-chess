...
//=====================================================
// V2 - Modern paragraph-based documentation style
//=====================================================

function docoGenMethodListV2(obj) {
...
function getSignatureV2(method) {
  var params = [];
...
function formatParamV2(param, indent) {
  indent = indent || 0;
  ...
  if (param.children && param.children.length) {
    for (var i = 0; i < param.children.length; i++) {
      result += formatParamV2(param.children[i], indent + 1);
    }
  }
...
function formatParamsV2(params) {
  ...
  for (var i = 0; i < params.children.length; i++) {
    result += formatParamV2(params.children[i], 0);
  }
...
// Format return value
function formatReturnV2(returnVal) {
  if (!returnVal) {
    return '*None*\n';
  }
  ...
  if (isArr(returnVal) && returnVal.length === 2) {
    ...
    if (returnVal[0].children && returnVal[0].children.length) {
      for (var i = 0; i < returnVal[0].children.length; i++) {
        result += formatParamV2(returnVal[0].children[i], 0);
      }
    }
...
function docoGenMethodV2(methodKey, method) {
  var res = '';
  
  // Method header with anchor
  res += '---\n\n';
  res += '### `' + getSignatureV2(method) + '`\n\n';
  ...
  // Parameters section (collapsible if complex - more than 5 total nested params)
  var totalParams = countNestedParams(method.params);
  var hasComplexParams = totalParams > 5;
  
  if (hasComplexParams) {
    res += '<details>\n<summary><strong>Parameters</strong> <em>(' + totalParams + ' total)</em></summary>\n\n';
  } else {
    res += '**Parameters:**\n\n';
  }
  
  res += formatParamsV2(method.params);
  ...
  // Returns section
  res += '**Returns:**\n\n';
  res += formatReturnV2(method.returnVal);
  res += '\n';
...
function docoGenMethodsV2(obj) {
  ...
  for (var i = 0, len = curr_table.length; i < len; i++) {
    ...
    if (method.description) {
      rtn.push(docoGenMethodV2(methodKey, method));
    }
  }
...
function docoGenQuickRefV2(obj) {
  var result = '';
