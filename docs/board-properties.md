...
**legalUciTree** | Object | Collection with all the **legal uci moves** divided into arrays (one array per each **square bos** that with at least one legal move).<hr>Examples:<ul><li>`board.legalUciTree //{a2 : ["a2a3", ...], b2 : ...}`</li><li>`board_after_e4.legalUciTree //{a7 : ["a7a6", ...], b7 : ...}`</li></ul>
