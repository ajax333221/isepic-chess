...
**enPassantBos** | String | Holds the **En passant bos** square (if any) or an empty string.<br><br>It will also hold an empty string if there isn't a pawn that can legally make the En passant capture without exposing the King.<hr>Examples:<ul><li>`board_after_e4 //""`</li><li>`board_after_e4_e6_e5_f5.enPassantBos //"f6"`</li><li>`board_no_enpass.enPassantBos //""`</li></ul>
