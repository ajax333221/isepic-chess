...
(function (windw?, expts?, defin?) {
  var Ic = (function (_WIN?: any) {
    var _VERSION: string = '8.7.2';

    var _SILENT_MODE: boolean = true;
    var _BOARDS: Ts.Boards = {};
    var _EMPTY_SQR: Ts.EmptyVal = 0;
    var _PAWN_W: Ts.WpVal = 1;
    var _KNIGHT_W: Ts.WnVal = 2;
    var _BISHOP_W: Ts.WbVal = 3;
    var _ROOK_W: Ts.WrVal = 4;
    var _QUEEN_W: Ts.WqVal = 5;
    var _KING_W: Ts.WkVal = 6;
    var _PAWN_B: Ts.BpVal = -1;
    var _KNIGHT_B: Ts.BnVal = -2;
    var _BISHOP_B: Ts.BbVal = -3;
    var _ROOK_B: Ts.BrVal = -4;
    var _QUEEN_B: Ts.BqVal = -5;
    var _KING_B: Ts.BkVal = -6;
    ...
    var _TEST_COLLISION_OP_CANDIDATE_MOVES: Ts.TestCollisionOpCandidateMoves = 1;
    var _TEST_COLLISION_OP_IS_ATTACKED: Ts.TestCollisionOpIsAttacked = 2;
    ...
    function _joinedWrapmoveHelper(mov: Ts.Mov, p?: Ts.OptionalParam): null | Ts.MoveFromTo {
      ...
      block: {
        ...
        // @ts-ignore
        let fromTo: Ts.MoveFromTo = temp3;
    ...
    function _updateFenAndMisc(sliced_fen_history?): void {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        for (let j = 0; j < 8; j++) {
          //0...7
          let temp = that?.legalMovesHelper?.([i, j]);
          let len = temp.uciMoves.length;
          ...
          for (let k = 0; k < len; k++) {
            ...
            if (temp.isPromotion) {
              for (let m = _KNIGHT_W; m <= _QUEEN_W; m++) {
                ...
                // @ts-ignore
                let uci_promotion_move: Ts.UciPromotionMove = temp2 + toBal(m).toLowerCase();
    ...
    function _legalMovesHelper(target_qos: Ts.Qos): Ts.LegalMovesHelper {
      ...
      function _candidateMoves(qos: Ts.Qos, piece_direction?, as_knight?, max_shifts?, allow_capture?) {
        //uses: that
    ...
    function _isEqualBoard(to_woard: string | Ts.Board): boolean {
      let that: Ts.Board = this;
    ...
    function _cloneBoardFrom(from_woard: string | Ts.Board): boolean {
      let that: Ts.Board = this;
    ...
    function _cloneBoardTo(to_woard: string | Ts.Board): boolean {
      let that: Ts.Board = this;
      ...
      for (let i = 0; i < 8; i++) {
        ...
        for (let j = 0; j < 8; j++) {
          ...
          if (current_square.isBishop) {
            let current_side: Ts.BishopCounts = current_square.sign > 0 ? rtn.w : rtn.b;
    ...
    function _draftMove(mov?, p?: Ts.OptionalParam) {
      var that,
        temp,
        temp2,
    ...
    function _playMove(mov?, p?: Ts.OptionalParam, sliced_fen_history?) {
      var i,
        that,
        temp,
        temp2,
    ...
    function _playRandomMove(p?: Ts.OptionalParam, sliced_fen_history?) {
    ...
    function initBoard(p?: Ts.OptionalParam) {
      var temp,
        board_created,
        board_name,
        fen_was_valid,
