  ...
  var Ic = (function (_WIN?: any) {
    ...
    function _getSquare(qos: Ts.Qos, p?: Ts.OptionalParam): null | Ts.Square {
      ...
      function _squareHelper(my_square: Ts.Square, is_unreferenced: boolean): Ts.Square {
        ...
        if (is_unreferenced) {
          // @ts-ignore
          let square_pos: Ts.SquarePos = toPos(my_square.pos);
          // @ts-ignore
          let square_rank_pos: Ts.SquareRankPos = getRankPos(my_square.pos);
          // @ts-ignore
          let square_file_pos: Ts.SquareFilePos = getFilePos(my_square.pos);
          // @ts-ignore
          let square_rank_bos: Ts.SquareRankBos = getRankBos(my_square.pos);
          // @ts-ignore
          let square_file_bos: Ts.SquareFileBos = getFileBos(my_square.pos);

          rtn_square = {
            pos: square_pos, //unreference
            bos: my_square.bos,
    ...
    function _refinedFenTest(): string {
      ...
      block: {
        let active_side: Ts.WhiteInfo | Ts.BlackInfo = that[that.activeColor];
        let non_active_side: Ts.WhiteInfo | Ts.BlackInfo = that[that.nonActiveColor];
    ...
    function _undoMoves(decrease_by?): null | Ts.Move[] {
      ...
      block: {
        ...
        for (let i = 0; i < decrease_by; i++) {
          //0<decrease_by
          let move: Ts.Move = that.moveList[that.moveList.length - i - 1];
