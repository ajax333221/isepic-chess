...
**fenApply**(<br>*fen*,<br>`"legalMoves"`,<br>[<br>*piece_qos*<br>]<br>) | <ul><li>fen (String)</li><li>`"legalMoves"` (String)</li><li>piece_qos:<ul><li>**pieceBos** (String)</li><li>**piecePos** (Array)</li></ul></li></ul> | Success:<ul><li>**squarePosArray** (Array)</li></ul><hr>Error:<ul><li>Array: `[]`</li></ul> | Returns a **square pos array** that a **piece** (from the **active** color to move) can move to.<br><br>Passing a `piece_qos` square with a **piece val** of `0` or a **non active sign** (that is, an empty square or a square occupied by a **piece** from the **non active** color to move) will result in a `[]`.<hr>Examples:<ul><li>`Ic.fenApply("8/8/8/4k3/8/8/r1R1K3/8 w - - 0 1", "legalMoves", ["c2"]) //[[6, 1], [6, 0], [6, 3]]`<ul><li>:zap:**Tip:** to convert Arrays of **pos** to **bos** use<br>`Ic.mapToBos([...])` or<br>`[...].map(x => Ic.toBos(x))` to get `["b2", "a2", "d2"]`.</li></ul></li><li>`Ic.fenApply("8/8/8/4k3/8/8/r1R1K3/8 w - - 0 1", "legalMoves", ["a2"]) //[]`</li><li>`Ic.fenApply("0invalidfen0", "legalMoves", ["a1"]) //[]`</li></ul>
**fenApply**(<br>*fen*,<br>`"isLegalMove"`,<br>[<br>*initial_qos*,<br>*final_qos*<br>]<br>) | <ul><li>fen (String)</li><li>`"isLegalMove"` (String)</li><li>initial_qos:<ul><li>**pieceBos** (String)</li><li>**piecePos** (Array)</li></ul></li><li>final_qos:<ul><li>**squareBos** (String)</li><li>**squarePos** (Array)</li></ul></li></ul> | Success:<ul><li>Boolean</li></ul><hr>Error:<ul><li>Boolean: `false`</li></ul> | Test to see if a move from `initial_qos` to `final_qos` is valid or not.<hr>Examples:<ul><li>`Ic.fenApply("8/8/8/4k3/8/8/r1R1K3/8 w - - 0 1", "isLegalMove", ["c2", "a2"]) //true`</li><li>`Ic.fenApply("8/8/8/4k3/8/8/r1R1K3/8 w - - 0 1", "isLegalMove", ["a2", "c2"]) //false`</li><li>`Ic.fenApply("0invalidfen0", "isLegalMove", ["a1", "a2"]) //false`</li></ul>
