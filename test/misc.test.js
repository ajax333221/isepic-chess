  ...
  describe('Regression tests', () => {
    ...
    describe('Promotion related', () => {
      ...
      test('K should change to Q', () => {
        expect(
          Ic.fenApply(shared_fen, 'playMove', ['b7-a8', { promoteTo: 'K' }], { skipFenValidation: true }).san
        ).toBe('bxa8=Q');
      });

      test('P should change to N', () => {
        expect(
          Ic.fenApply(shared_fen, 'playMove', ['b7-a8', { promoteTo: 'P' }], { skipFenValidation: true }).san
        ).toBe('bxa8=N');
      });

      test('valid promoteTo option should overwrite SAN', () => {
        expect(
          Ic.fenApply(shared_fen, 'playMove', ['bxa8=R', { promoteTo: 'N' }], { skipFenValidation: true }).san
        ).toBe('bxa8=N');
      });

      test('valid promoteTo option should overwrite UCI', () => {
        expect(
          Ic.fenApply(shared_fen, 'playMove', ['b7a8r', { promoteTo: 'N' }], { skipFenValidation: true }).san
        ).toBe('bxa8=N');
      });

      test('invalid promoteTo option should NOT overwrite SAN', () => {
        expect(
          Ic.fenApply(shared_fen, 'playMove', ['bxa8=R', { promoteTo: 'W' }], { skipFenValidation: true }).san
        ).toBe('bxa8=R');
      });

      test('invalid promoteTo option should NOT overwrite UCI', () => {
        expect(
          Ic.fenApply(shared_fen, 'playMove', ['b7a8r', { promoteTo: 'W' }], { skipFenValidation: true }).san
        ).toBe('bxa8=R');
      });
      ...
      test('board.promoteTo in isMockMove from fenApply is lowest priority', () => {
        expect(Ic.fenApply(shared_fen, 'playMove', ['bxa8'], { promoteTo: 'B', skipFenValidation: true }).san).toBe(
          'bxa8=B'
        );
        expect(Ic.fenApply(shared_fen, 'playMove', ['bxa8=X'], { promoteTo: 'B', skipFenValidation: true }).san).toBe(
          'bxa8=B'
        );
        expect(Ic.fenApply(shared_fen, 'playMove', ['bxa8=N'], { promoteTo: 'B', skipFenValidation: true }).san).toBe(
          'bxa8=N'
        );
        expect(
          Ic.fenApply(shared_fen, 'playMove', ['bxa8=N', { promoteTo: 'R' }], {
            promoteTo: 'B',
            skipFenValidation: true,
          }).san
        ).toBe('bxa8=R');
        expect(
          Ic.fenApply(shared_fen, 'playMove', ['bxa8=N', { promoteTo: 'X' }], {
            promoteTo: 'B',
            skipFenValidation: true,
          }).san
        ).toBe('bxa8=N');
        expect(
          Ic.fenApply(shared_fen, 'playMove', ['bxa8=X', { promoteTo: 'X' }], {
            promoteTo: 'B',
            skipFenValidation: true,
          }).san
        ).toBe('bxa8=B');
    ...
    describe('Enpassant related', () => {
      test('enpassant capture made with a non-pawn piece', () => {
        expect(
          Ic.fenApply('rnbqkbnr/1pp1pp2/p7/2PpQ1p1/2K4p/3P4/PP2PPPP/RNB2BNR w kq d6 0 8', 'legalMoves', ['e5'], {
            skipFenValidation: true,
          }).sort()
        ).toEqual(['d5'].sort());
      });
      ...
      test('taking enpassant results in self check', () => {
        expect(
          Ic.fenApply('8/8/1k6/8/2pP4/8/8/6BK b - d3 0 1', 'isLegalMove', ['c4-d3'], { skipFenValidation: true })
        ).toBe(false);
        expect(
          Ic.fenApply('r1b1kbnr/ppp3pp/3q4/P2nPp2/3p4/7K/1PP2PP1/RNBQ1BNR w kq f6 0 10', 'legalMoves', ['e5'], {
            skipFenValidation: true,
          }).sort()
        ).toEqual(['d6', 'e6'].sort());
      });
      ...
      test('missing option to remove check via enpassant', () => {
        expect(
          Ic.fenApply('8/8/8/3k4/3pP3/8/8/7K b - e3 0 1', 'isLegalMove', ['d4-e3'], { skipFenValidation: true })
        ).toBe(true);
      });
...
4. d4 Nh6 {}{} 5. Rg1`;
    ...
    test('Checkmate and 50-move rule', () => {
      ...
      expect(Ic.fenGet(fen, 'isCheckmate isFiftyMove inDraw', { skipFenValidation: true })).toEqual({
        isCheckmate: true,
        isFiftyMove: true,
        inDraw: false,
      });
    ...
    test('Should play UCI instead of SAN in b.playMoves() + b.undoMoves()', () => {
      ...
      game_pgn =
        '1. b4 b5 2. c4 c5 3. d4 d5 4. Qd2 a6 5. Nh3 Ra7 6. Na3 Ra8 7. Nb1 Ra7 8. Qd1 Ra8 9. Bd2 Ra7 10. Bh6 Ra8 11. Bd2';
